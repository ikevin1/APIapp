...
namespace QuizApp1
{
    ...
    {
        IList<Quizy> quizy;
        private IList<Characters> characters;
        ...
        public async void resultClicked(object sender, EventArgs args)
        {
            //string xter;
            int xterPoint = 1;
            YoResults.Text = $"Hello {Name.Text}, YoResults is: { currentScore}";
            ...
            {
                //QuizData quizData = await App.GetTasksAsync(c.CharacterAPI);
                if (xterPoint < currentScore)
                {
                    players.Text = $" {Name.Text}, you represent {c.Name}- {c.CharacterAPI} character - {displayAPIInfo(c)}. :)";
                   // await DisplayAlert("" + currentScore, c.Name + c.CharacterAPI, "OK");
        ...
        {
            QuizData quizData = await App.QM.GetTasksAsync(c.CharacterAPI);

            showText.Text = "You are " + quizData.Name;
            //if (xterPoint < currentScore)
            //{
            //    currentScore += 5;
            //    players.Text = $" {Name.Text}, you represent {Characters.All} character - Luke Skywalker. :)";
